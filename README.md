...
The first request successfully removes the room from the system.
Any subsequent DELETE requests for the same room will not alter the system state further, as the resource no longer exists.
...
The reading is added to the sensor’s historical collection.
The parent sensor’s currentValue is updated to reflect the latest value.
